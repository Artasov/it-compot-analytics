...
tokens.default_token_manager(
    ...
    client_secret=AMO_SECRET,  # Ваш клиентский секрет
    subdomain=AMO_SUBDOMAIN,  # Ваш субдомен
    redirect_url=None,  # Не нужен для постоянного токена
    storage=tokens.FileTokensStorage(directory_path='.'),  # по умолчанию FileTokensStorage
)
# tokens.default_token_manager.init(AMO_TOKEN)
DATABASES = {
    'default': {
        ...
        'HOST': env('SQL_HOST') if not DEV else 'localhost',
        'PORT': env('SQL_PORT'),
    }
}
